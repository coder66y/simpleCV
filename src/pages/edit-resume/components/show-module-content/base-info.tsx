      ...
      {Object.entries(baseInfo || {})?.map(([key, value]) => {
        ...
        if (key === 'photo') {
          return (
            <div className="avatar-wrapper">
              {baseInfo?.photoShow ? (
                <img className="avatar" key={key} src={baseInfo?.photo} />
              ) : null}
            </div>
          );
        }
        if (key === 'birthday') {
          return (
            <ReadItem
              key={key}
              span={colSpan1}
              className="read-item-birthday"
              value={baseInfo?.birthday ? getIntlDate(baseInfo?.birthday) : ''}
              label={getIntlText('birthday')}
            />
          );
        }
        if (key === 'height') {
          return (
            <ReadItem
              span={colSpan1}
              className="read-item-heightWeight"
              value={
                baseInfo?.height &&
                baseInfo?.weight && (
                  <>
                    <ReadItem
                      value={baseInfo?.height}
                      needCol={false}
                      label={`${getIntlText('height')}/${getIntlText('weight')}`}
                      suffix={`cm ${' '}/${' '}`}
                    />
                    <ReadItem
                      value={baseInfo?.weight}
                      needCol={false}
                      suffix="kg"
                    />
                  </>
        ...
        if (
          ...
          return (
            <ReadItem
              key={key}
              span={colSpan1}
              value={_value ? getIntlText(_value) : ''}
              label={getIntlText(key)}
            />
          );
        }
        ...
        return (
          <ReadItem
            key={key}
            span={colSpan1}
            className={`read-item-${key}`}
            suffix={key === 'age' ? '岁' : undefined}
            value={_value ?? ''}
            label={getIntlText(key)}
          />
        );
