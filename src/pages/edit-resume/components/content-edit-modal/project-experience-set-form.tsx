...
/** 表单初始值 */
export interface IProjectExperienceSetFormValues
  extends Omit<IProjectExperienceValues, 'start' | 'end'> {
  start?: Dayjs;
  end?: Dayjs;
}
...
/** 基础表单 */
function ProjectExperienceBaseSetForm(
  props: IProjectExperienceBaseSetFormProps,
) {
...
function ProjectExperienceSetForm(props: IProjectExperienceSetFormProps) {
  ...
  const handleChange = (
    values: IProjectExperienceSetFormValues,
    index?: number,
  ) => {
    let newList = [];
    const newValues = {
    ...
    if (Number(index ?? -1) >= 0) {
      ...
      newList = [...projectExperience, newValues];
    }
    dispatch({
  ...
  return (
    <div className="common-list-set-form-wrapper">
      ...
      <Button type="primary" ghost className="add-btn" onClick={onAdd}>
        ...
        {
          infoModuleList?.find(
            (item) => item.key === ContentConfigKeyEnum.PROJECT_EXPERIENCE,
          )?.title
        }
      </Button>
    </div>
