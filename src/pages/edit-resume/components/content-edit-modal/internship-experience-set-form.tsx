...
/** 表单属性 */
export interface IInternshipExperienceBaseSetFormProps {
  initValues?: IInternshipExperienceSetFormValues;
  onChange?: (values: IInternshipExperienceSetFormValues, index: number) => void;
  index: number;
  length?: number;
...
/** 表单初始值 */
export interface IInternshipExperienceSetFormValues
  extends Omit<IInternshipExperienceValues, 'start' | 'end'> {
  start?: Dayjs;
  end?: Dayjs;
}
...
/** 基础表单 */
function InternshipExperienceBaseSetForm(props: IInternshipExperienceBaseSetFormProps) {
      ...
      <Row justify="end">
        <Space>
          {index > 0 && (
            <Button
              type="primary"
              id={index === length - 1 ? 'lastOne' : ''}
              ghost
              onClick={() => handleSort(SortTypeEnum.UP)}
            >
              上移
          ...
          {length > 1 && index < length - 1 && (
            <Button type="primary" ghost onClick={() => handleSort(SortTypeEnum.DOWN)}>
              下移
              <i className="iconfont">&#xe8f6;</i>
            </Button>
          )}
          {length > 1 && (
            <Button type="primary" ghost onClick={() => handleSort(SortTypeEnum.DELETE)}>
              删除
              <i className="iconfont">&#xe600;</i>
            </Button>
          )}
...
function InternshipExperienceSetForm(props: IInternshipExperienceSetFormProps) {
  ...
  const handleChange = (values: IInternshipExperienceSetFormValues, index?: number) => {
    ...
    if (Number(index ?? -1) >= 0) {
      newList = internshipExperience?.map((item, idx) => {
        if (index === idx) {
          return {
            ...item,
            ...newValues,
      ...
      newList = [...internshipExperience, newValues];
    }
    dispatch({
  ...
  return (
    <div className="common-list-set-form-wrapper">
      {internshipExperience?.map?.((item, index) => {
        return (
          <InternshipExperienceBaseSetForm
            key={item.id}
            onChange={(values: IInternshipExperienceSetFormValues) => {
              handleChange(values, index);
            }}
            length={internshipExperience.length}
            onSort={onSort}
            initValues={{
              ...item,
      ...
      <Button type="primary" ghost className="add-btn" onClick={onAdd}>
        新增
        {
          infoModuleList?.find(item => item.key === ContentConfigKeyEnum.INTERNSHIP_EXPERIENCE)
            ?.title
        }
      </Button>
    </div>
