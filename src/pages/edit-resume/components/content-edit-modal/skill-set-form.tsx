...
const SkillsBarSetForm = (props: ISkillsBarSetFormProps) => {
  ...
  return (
    ...
    >
      ...
      <Form.Item>
        <i className="iconfont" onClick={onDelete}>
          &#xe600;
        </i>
      </Form.Item>
    </Form>
...
function SkillsSetForm(props: ISkillsSetFormProps) {
  ...
  const title =
    infoModuleList?.find((item) => item.key === ContentConfigKeyEnum.SKILLS)
      ?.title ?? '';
  ...
  const { run: _onChangeContent } = useDebounceFn(
    (value) => {
      ...
      <Row>
        <Space>
          ...
          <Button type="primary" ghost onClick={onAdd}>
            <i className="iconfont">&#xe60c;</i>添加{title}
          </Button>
        </Space>
      </Row>
      <Row>
        {data.map((item, index) => (
          <Col span={colSpan2} key={item.name + index}>
            <SkillsBarSetForm
              index={index}
              values={item}
              onValuesChange={(v) => onListValueChange(v, index)}
