...
/** 表单初始值 */
export interface ISchoolExperienceSetFormValues
  extends Omit<ISchoolExperienceValues, 'start' | 'end'> {
  start?: Dayjs;
  end?: Dayjs;
}
...
function SchoolExperienceSetForm(props: ISchoolExperienceSetFormProps) {
  ...
  const handleChange = (
    values: ISchoolExperienceSetFormValues,
    index?: number,
  ) => {
    let newList = [];
    const newValues = {
    ...
    if (Number(index ?? -1) >= 0) {
      ...
      newList = [...schoolExperience, newValues];
    }
    dispatch({
  ...
  return (
    <div className="common-list-set-form-wrapper">
      ...
      <Button type="primary" ghost className="add-btn" onClick={onAdd}>
        ...
        {
          infoModuleList?.find(
            (item) => item.key === ContentConfigKeyEnum.SCHOOL_EXPERIENCE,
          )?.title
        }
      </Button>
    </div>
