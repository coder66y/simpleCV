...
/** 表单初始值 */
export interface IWorkExperienceSetFormValues
  extends Omit<IWorkExperienceValues, 'start' | 'end'> {
  start?: Dayjs;
  end?: Dayjs;
}
...
function WorkExperienceSetForm(props: IWorkExperienceSetFormProps) {
  ...
  const handleChange = (
    values: IWorkExperienceSetFormValues,
    index?: number,
  ) => {
    let newList = [];
    const newValues = {
    ...
    if (Number(index ?? -1) >= 0) {
      ...
      newList = [...workExperience, newValues];
    }
    dispatch({
  ...
  return (
    <div className="common-list-set-form-wrapper">
      ...
      <Button type="primary" ghost className="add-btn" onClick={onAdd}>
        ...
        {
          infoModuleList?.find(
            (item) => item.key === ContentConfigKeyEnum.WORK_EXPERIENCE,
          )?.title
        }
      </Button>
    </div>
