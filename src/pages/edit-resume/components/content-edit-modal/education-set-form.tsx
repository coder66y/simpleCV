...
/** 教育信息表单初始值 */
export interface IEducationSetFormValues
  extends Omit<IEducationInfoValues, 'start' | 'end'> {
  start?: Dayjs;
  end?: Dayjs;
}
...
function EducationSetForm(props: IEducationSetFormProps) {
  ...
  const handleChange = (values: IEducationSetFormValues, index?: number) => {
    ...
    if (Number(index ?? -1) >= 0) {
      ...
      newEducationInfo = [...educationInfo, newValues];
    }
    dispatch({
  ...
  return (
    <div className="common-list-set-form-wrapper">
      ...
      <Button type="primary" ghost className="add-btn" onClick={onAdd}>
        ...
        {
          infoModuleList?.find(
            (item) => item.key === ContentConfigKeyEnum.EDUCATION,
          )?.title
        }
      </Button>
    </div>
