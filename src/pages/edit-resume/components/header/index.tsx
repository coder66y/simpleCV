 ...
 * @LastEditors: luyi
 * @LastEditTime: 2025-03-21 11:22:12
 */
...
import { getLocalStorage, setLocalStorage, simpleCVCacheKey } from "@/utils/local-storage";
    ...
    input.onchange = (e) => {
      ...
      reader.onload = (e) => {
        ...
        if(typeof result === 'string') {
          try {
            setLocalStorage(simpleCVCacheKey, JSON.parse(result));
  ...
  return (
    <ThreeColumnLayout gap={20} className={`edit-cv-title ${className}`}>
      ...
      <Space>
        ...
        <Button type="primary" ghost onClick={onExportJSON} loading={exportLoading}>导出JSON</Button>
        <Button type="primary" ghost onClick={onImportJSON} loading={importLoading}>导入JSON</Button>
        <Tooltip title="">
