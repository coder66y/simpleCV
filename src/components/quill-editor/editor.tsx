...
export interface IQuillEditorProps extends QuillOptions {
  ...
  value?: Value;
  defaultValue?: Value;
  onChange?: (value: Value, delta: Delta, source: EmitterSource, editor: Quill) => void;
  readOnly?: boolean;
...
const Editor = (props: IQuillEditorProps) => {
  const { value, children, readOnly, defaultValue, className, preserveWhitespace, onChange } =
    props;
  ...
  const onEditorChangeSelection = (nextSelection: Range, source: EmitterSource, editor: Quill) => {
    if (!editor) return;
  ...
  const getEditingArea = (): HTMLElement => {
    ...
    if (!editingArea || !(editingArea instanceof Element)) {
